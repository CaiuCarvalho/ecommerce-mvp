...
function renderPage() {
  return renderWithProviders(<MinhaConta />)
}
...
describe('MinhaConta page', () => {
  ...
  it('shows "Você ainda não realizou nenhum pedido" when no orders exist', async () => {
    supabaseMock.from = vi.fn(() => {
      const b = {}
      const chain = ['select', 'eq', 'order', 'limit']
      for (const m of chain) b[m] = () => b
    ...
    renderPage()
    await waitFor(() => expect(screen.getByText(/Você ainda não realizou nenhum pedido/)).toBeInTheDocument())
  })
})
