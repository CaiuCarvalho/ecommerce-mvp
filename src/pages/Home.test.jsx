...
describe('Home page', () => {
  ...
  it('shows "Esgotado" for out_of_stock products and Add button for available ones', async () => {
    renderHome()
    await waitForElementToBeRemoved(() => screen.queryByText(/Carregando produtos/))
    expect(screen.getByRole('button', { name: /Adicionar a Sacola/i })).toBeInTheDocument()
  ...
  it('clicking a category filter filters the products', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('Fone Bluetooth')).not.toBeInTheDocument())
    expect(screen.getByText('Ração Premium')).toBeInTheDocument()
  })
  ...
  it('Add to cart calls addItem with product data', async () => {
    ...
    expect(addItem.current.mock.calls[0][0]).toMatchObject({ id: 'p1', name: 'Fone Bluetooth', price: 79.9 })
  })
})
